...
$color_arr = array("玫瑰金"=>"R","黄"=>"Y","白"=>"W","分色"=>"C","玫瑰白"=>"RW","玫瑰黄"=>"RY","黄白"=>"YW","彩金"=>"H");
$color_value_arr = array("玫瑰金"=>3,"黄"=>2,"白"=>1,"分色"=>4,"黄白"=>5,"玫瑰白"=>6,"玫瑰黄"=>7,"彩金"=>8);

//读取镶口、金重信息
$sql = "select * from `app_xiangkou` where `style_sn` in ('W5731') order by `x_id` desc;";
...
//所有款式信息
$sql = "select `style_id`,`style_sn`,`style_name`,`product_type`,`style_type`,`is_made`,`check_status` from `base_style_info` where style_sn in ('W9971','W7443','W5731','KLRX010562','KLRW029026','B9446','KLRM028916','A1023','KLRX014759') order by `style_id` desc;";
$result = $mysqli->query($sql);
$s_styleinfo = array();
...
//获取款式属性值信息
$sql = "select `style_id`,`attribute_id`,`attribute_value` from `rel_style_attribute` where style_sn in ('W9971','W7443','W5731','KLRX010562','KLRW029026','B9446','KLRM028916','A1023','KLRX014759');";
$result = $mysqli->query($sql);
...
//print_r($yanse_data);die;
    ...
    foreach ($yanse_data as $ys_key => $ys_val) {
        ...
        //循环指圈
        foreach ($cut_finger as $ct_k => $ct_v) {
            # code...
            $shoucun = $ct_v;//手寸
...
function getListStyleGoods(){
    global $mysqli;
    $sql = "select * from front.`list_style_goods` where style_sn in ('W9971','W7443','W5731','KLRX010562','KLRW029026','B9446','KLRM028916','A1023','KLRX014759');";
    $arr = $mysqli->query($sql);
