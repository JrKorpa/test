...
$sql = "select `x`.* from `base_style_info` `s` inner join `app_xiangkou` `x` on `s`.`style_sn` = `x`.`style_sn` where `s`.style_sn in('KLRW030288-18K-W-100-17')";
//
...
foreach ($s_xiangkou as $key => $val) {
    ...
    if(!isset($style_data[$val['style_id']])){//不存在
        ...
        file_put_contents("ztxl.log",$sxl."\r\n",FILE_APPEND);
        continue;
    }
