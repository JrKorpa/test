...
$sql = "select `x`.* from `base_style_info` `s` inner join `app_xiangkou` `x` on `s`.`style_sn` = `x`.`style_sn` where `s`.style_sn in('KLDW027054',
'KLPW024276',
'KLPW027055')";
//
...
foreach ($s_xiangkou as $key => $val) {
    ...
    if(isset($style_attrval[$val['style_sn']][$cz_id])){

        $cz_str = '';
        $cz_str = $style_attrval[$val['style_sn']][$cz_id];
        if($cz_str != ''){
            $cz_info = array();
    ...
    if(isset($style_attrval[$val['style_sn']][$ys_id])){

//var_dump($val['style_id'],$ys_id,$style_attrval[$val['style_id']][$ys_id]);die;

        $ys_str = '';
        $ys_str = $style_attrval[$val['style_sn']][$ys_id];
...
function create_goods_insert($styleNew, $jinZongInfo, $caizhiInfo, $yanse_data)
{
    ...
    //var_dump($style_sn);die;
    //切分手寸
    $cut_finger = array(0=>0);
    if($finger != 0){
...
$sxl = $val['style_sn'].'副石1重为空';
        ...
        //循环指圈
        # code...
        foreach ($cut_finger as $key => $shoucun) {
...
function deleteListstylegoods($sn){
    global $mysqli;
    $sql = "delete from `list_style_goods` where goods_sn = '{$sn}';";
