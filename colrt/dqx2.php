...
$db = new MysqlDB($new_conf);
	...
	foreach($ret as $r){
		...
		foreach($names_2 as $u) {
			...
			$id = $model['id'];
			$sql = "
            INSERT INTO user_button_permission(user_id, parent_id, permission_id) values({$id}, 1692, 1764);
            INSERT INTO user_button_permission(user_id, parent_id, permission_id) values({$id}, 1692, 1765);
            INSERT INTO user_operation_permission(user_id, parent_id, permission_id) values({$id}, 1692, 1762);
            INSERT INTO user_operation_permission(user_id, parent_id, permission_id) values({$id}, 1692, 1763);
			";
...
/*
DELETE FROM `user_button_permission` WHERE `user_id` = 13304 AND `parent_id` = 1692 AND `permission_id` = 1764;
DELETE FROM `user_button_permission` WHERE `user_id` = 13304 AND `parent_id` = 1692 AND `permission_id` = 1765;
DELETE FROM `user_operation_permission` WHERE `user_id` = 13304 AND `parent_id` = 1692 AND `permission_id` = 1762;
DELETE FROM `user_operation_permission` WHERE `user_id` = 13304 AND `parent_id` = 1692 AND `permission_id` = 1763;
*/
