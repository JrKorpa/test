...
//读取镶口、金重信息
$sql = "select * from `app_xiangkou` where `style_sn` in('KLRM028916','B9446','KLRW029026','A1023','KLRW019287','KLRW019285','W9971','W7443') order by `x_id` desc;";
...
//所有款式信息
$sql = "select `style_id`,`style_sn`,`style_name`,`product_type`,`style_type`,`is_made`,`check_status` from `base_style_info` order by `style_id` desc;";
$result = $mysqli->query($sql);
$s_styleinfo = array();
...
//print_r($yanse_data);die;
    ...
    foreach ($yanse_data as $ys_key => $ys_val) {
        ...
        foreach ($cut_finger as $ct_k => $ct_v) {
            ...
            delete_goods($style_id,$stone,$shoucun,$ys_key);
...
//删除款的旧商品数据
function delete_goods($style_id,$xiangkou,$shoucun,$yanse)
{
    # code...
    global $mysqli;
    if($style_id == '' || $xiangkou == '' || $shoucun == '' || $yanse == ''){

        return false;
    }

    $sql = "delete from `list_style_goods` where `style_id` = {$style_id} and `xiangkou` = '{$xiangkou}' and `shoucun` = '{$shoucun}' and `yanse` = {$yanse}";
    $result = $mysqli->query($sql);
...
function insert_goods_info($data){
    ...
    echo $sql = "insert into `list_style_goods` (".implode(',',$keys).") values('".implode("','",$values)."');";
    $result = $mysqli->query($sql);
    echo "<br>";
    return $result;
}