...
foreach ($res as $value) {
    ...
    if($style_info['style_sex']==3){
        $err_style_sn[]= $style_info['style_sn'];
    }
...
//判断是否有中性戒指；
if(!empty($err_style_sn)){
    $str = '';
    $err_style_sn_s = array();
    $err_style_sn_s = array_unique($err_style_sn);
    $str = implode('<br/>', $err_style_sn_s);
    //echo '款式性别为中性：<br/>'.$str;die;
}
