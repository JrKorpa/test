 ...
 * @Last Modified time: 2015-08-19 11:17:07
 */
...
$sql = "select * from `app_xiangkou` where `stone` = 0.00;";
...
foreach ($style_data as $key => $value) {
    # code...
    //print_r($value);die;
    $fak[$value['style_sn']] = $value['stone'];
}
$dd = array();
foreach ($fak as $key => $value) {
    # code...
    $sql = "select * from `list_style_goods` where `style_sn` = '{$key}'";
    $result = $mysqli->query($sql);
    $fak_list = array();
    $fak_list = con($result);
    if(empty($fak_list)){
