...
define('ROOT_PATH', str_replace('warehouseBill_S.php', '', str_replace('\\', '/', __FILE__)));
...
$conNewS = new MysqlDB($conNewConf_S);
...
$sql="select order_id,order_sn from kela_order_part.ecs_order_info where 1 ";
//$sql.=" AND order_sn = '2015041113716' ";
$sql.=" AND order_time like '$data%'";
//echo $sql;die;
$all = $conOld->getAll($sql);
echo '<pre>';
print_r(count($all));die;
...
//6.销售单
    $jxc_order_Ssql="SELECT * FROM `jxc`.`jxc_order` o inner join `jxc`.`jxc_order_goods` og on `o`.`order_id`=`og`.`order_id` where `o`.`kela_order_sn` = '$order_sn' AND `o`.`type` = 'S' AND `o`.`status` = 2";
    $jxcInfo=$conOld->getAll($jxc_order_Ssql);
    //echo '<pre>';
    //print_r($jxcInfo);die;
    ...
    foreach($jxcInfo as $key => $jxc_info){
        if($key == 0 ){
            ...
            $warehouse_billInfo['bill_type'] = 'S';
            ...
            $warehouse_billInfo['from_bill_id'] = 0;//来源单据id
        }
    ...
    $bill_id = $conNewW->insert('warehouse_bill',$warehouse_billInfo);
    $conNewW->insert('toboss',array('tb'=>'warehouse_bill','zid'=>$bill_id,'order_sn'=>$order_sn,'order_id'=>$order_id));
    echo "3.销售单-{$bill_id}生成更新成功"."<hr>";
    foreach($warehouse_bill_goodsList as $warehouse_bill_goods)
    {
        ...
        $conNewW->insert('toboss',array('tb'=>'warehouse_bill_goods','zid'=>$id,'order_sn'=>$order_sn,'order_id'=>$order_id));
        echo "3.1 销售单-{$bill_id}-{$id}生成更新成功"."<hr>";
        ...
        if($rec_info)
        {
            ...
            if($row){
                ...
                $conNewW->insert('toboss',array('tb'=>'warehouse_goods','zid'=>$id,'order_sn'=>$order_sn,'order_id'=>$order_id));
